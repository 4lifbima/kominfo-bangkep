<x-app-layout>
    ...
    <div class="mb-6 animate-fade-up">
        ...
        <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Tambah Pegawai Baru</h2>
    </div>

    <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-up" style="animation-delay: 0.1s;">
        @csrf
            ...
            <div class="glass-panel p-6 rounded-2xl shadow-glass">
                ...
                <div class="mb-6 text-center">
                    <div class="relative w-32 h-32 mx-auto mb-4">
                        <img id="avatarPreview" src="https://ui-avatars.com/api/?name=User&background=random" class="w-full h-full rounded-full object-cover border-4 border-slate-100 dark:border-white/10">
                    ...
                    <p class="text-xs text-slate-500">Klik ikon kamera untuk upload foto.</p>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required placeholder="Contoh: Ratnasari, S.Sos" />
                        @error('name') <x-input-error class="mt-2" :messages="$message" /> @enderror
                    </div>
                    
                    <div>
                        <x-input-label for="nip" value="NIP / NIK (Opsional)" />
                        <x-text-input id="nip" name="nip" type="text" class="mt-1 block w-full" :value="old('nip')" placeholder="198xxxx" />
                        @error('nip') <x-input-error class="mt-2" :messages="$message" /> @enderror
                    </div>

                    <div>
                        <x-input-label for="position" value="Jabatan" />
                        <x-text-input id="position" name="position" type="text" class="mt-1 block w-full" :value="old('position')" required placeholder="Contoh: Kepala Dinas" />
            ...
            <div class="glass-panel p-6 rounded-2xl shadow-glass">
                ...
                <div class="space-y-3">
                    <div class="flex items-center gap-3">
                        ...
                        <x-text-input name="social_media[facebook]" type="url" class="block w-full text-sm" placeholder="URL Facebook" />
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-8 flex justify-center"><iconify-icon icon="ri:instagram-fill" class="text-pink-600 text-xl"></iconify-icon></div>
                        <x-text-input name="social_media[instagram]" type="url" class="block w-full text-sm" placeholder="URL Instagram" />
                    </div>
                    <div class="flex items-center gap-3">
                        ...
                        <x-text-input name="social_media[twitter]" type="url" class="block w-full text-sm" placeholder="URL Twitter/X" />
                    </div>
                    <div class="flex items-center gap-3">
                        <div class="w-8 flex justify-center"><iconify-icon icon="ri:linkedin-fill" class="text-blue-700 text-xl"></iconify-icon></div>
                        <x-text-input name="social_media[linkedin]" type="url" class="block w-full text-sm" placeholder="URL LinkedIn" />
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="glass-panel p-6 rounded-2xl shadow-glass" x-data="{ jobs: [['position' => '', 'institution' => '', 'year' => '']] }">
                    ...
                    <template x-for="(job, index) in jobs" :key="index">
                        <div class="grid grid-cols-1 md:grid-cols-7 gap-3 items-end p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 relative group">
                            <div class="md:col-span-3">
                                ...
                                <input type="text" :name="`job_history[${index}][position]`" x-model="job.position" class="block w-full rounded-md border-slate-300 dark:border-white/10 dark:bg-dark-bg text-sm focus:border-primary focus:ring-primary" placeholder="Posisi">
                            </div>
                            <div class="md:col-span-2">
                                <label class="text-xs text-slate-500 block mb-1">Instansi</label>
                                <input type="text" :name="`job_history[${index}][institution]`" x-model="job.institution" class="block w-full rounded-md border-slate-300 dark:border-white/10 dark:bg-dark-bg text-sm focus:border-primary focus:ring-primary" placeholder="Tempat">
                            </div>
                            <div class="md:col-span-2 relative">
                                <label class="text-xs text-slate-500 block mb-1">Tahun</label>
                                <input type="text" :name="`job_history[${index}][year]`" x-model="job.year" class="block w-full rounded-md border-slate-300 dark:border-white/10 dark:bg-dark-bg text-sm focus:border-primary focus:ring-primary" placeholder="20xx - 20xx">
                ...
                <p class="text-xs text-slate-400 mt-2 italic">* Data pertama akan dijadikan jabatan utama jika belum dipilih.</p>
            </div>

            <!-- Riwayat Pendidikan (Repeater) -->
            <div class="glass-panel p-6 rounded-2xl shadow-glass" x-data="{ edus: [['school' => '', 'degree' => '', 'year' => '']] }">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <template x-for="(edu, index) in edus" :key="index">
                        <div class="grid grid-cols-1 md:grid-cols-7 gap-3 items-end p-4 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/5 relative group">
                            <div class="md:col-span-3">
                                ...
                                <input type="text" :name="`education_history[${index}][school]`" x-model="edu.school" class="block w-full rounded-md border-slate-300 dark:border-white/10 dark:bg-dark-bg text-sm focus:border-primary focus:ring-primary" placeholder="Nama Sekolah">
                            </div>
                            <div class="md:col-span-2">
                                <label class="text-xs text-slate-500 block mb-1">Jurusan / Gelar</label>
                                <input type="text" :name="`education_history[${index}][degree]`" x-model="edu.degree" class="block w-full rounded-md border-slate-300 dark:border-white/10 dark:bg-dark-bg text-sm focus:border-primary focus:ring-primary" placeholder="S1 - Teknik...">
                            </div>
                            <div class="md:col-span-2 relative">
                                <label class="text-xs text-slate-500 block mb-1">Tahun</label>
                                <input type="text" :name="`education_history[${index}][year]`" x-model="edu.year" class="block w-full rounded-md border-slate-300 dark:border-white/10 dark:bg-dark-bg text-sm focus:border-primary focus:ring-primary" placeholder="20xx - 20xx">
