<x-app-layout>
    <x-slot:title>Tambah Data Pegawai</x-slot>
    ...
    <div class="flex items-center gap-4 animate-fade-up">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Tambah Pegawai</h2>
            <p class="text-slate-500 dark:text-slate-400 text-sm">Input data pegawai baru.</p>
        </div>
    </div>
        ...
        <form action="{{ route('employees.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5" placeholder="Contoh: Budi Santoso, S.Kom" required>
                    @error('name') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label for="nip" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">NIP (Opsional)</label>
                    <input type="text" name="nip" id="nip" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5" placeholder="Contoh: 19900101 201901 1 001">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="position" id="position" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5" placeholder="Contoh: Kepala Bidang TIK" required>
                </div>
                 <div>
             ...
             <div>
                ...
                <select name="status" id="status" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5">
                    ...
                    <option value="inactive">Inactive (Cuti/Keluar/Pensiun)</option>
                </select>
            </div>
