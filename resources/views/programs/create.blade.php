<x-app-layout>
    <x-slot:title>Tambah Program Kerja</x-slot>
    ...
    <div class="flex items-center gap-4 animate-fade-up">
        ...
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-white">Tambah Program Kerja</h2>
            <p class="text-slate-500 dark:text-slate-400 text-sm">Input data program kerja dan kegiatan baru.</p>
        </div>
    </div>
    ...
    <div class="glass-panel rounded-2xl p-6 shadow-glass animate-fade-up mt-6 max-w-3xl" style="animation-delay: 0.1s;">
        <form action="{{ route('programs.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="text" name="name" id="name" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5" placeholder="Contoh: Pengembangan Aplikasi SPBE" required>
            ...
            <!-- Description -->
             <div>
                ...
                <textarea name="description" id="description" rows="3" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5" placeholder="Contoh: Membangun modul e-office integrasi tanda tangan elektronik"></textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <input type="number" name="progress" id="progress" min="0" max="100" value="0" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5" required>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full rounded-lg border-slate-300 dark:border-white/10 bg-slate-50 dark:bg-white/5 text-sm focus:border-primary focus:ring focus:ring-primary/20 p-2.5">
                        <option value="Planned">Planned (Perencanaan)</option>
                        <option value="On Progress">On Progress (Sedang Berjalan)</option>
